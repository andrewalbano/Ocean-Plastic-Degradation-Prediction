...
def save_location_pre_processing(kernel, output_path=None):
    ...
    folder_name = f"SVM_{kernel}_results_hyperparam_testing"
...
# Selectes the features of the cv.results_ to return for each grid search
res_features_linear = ["rank_test_score",
                       "param_C",
                       "mean_test_score"
                       ] 

res_features_rbf= ["rank_test_score",
                       "param_C",
                       "param_gamma",
                       "mean_test_score"
                       ] 

res_features_poly= ["rank_test_score",
                       "param_degree",
                       "param_gamma",
                       "mean_test_score"
                       ] 
...
save_name_mapping = {'den (g/mL)':'density',
                    ...
                    '(Tw-Tg)/(LogP/SA)':'TWTG_LogPSA'
                    }

gamma_rbf = {'gamma': [0.01, 0.1, 1, 5, 7, 8, 9,10]}

C = {'C': [0.1, 1, 5, 7, 8, 9, 10, 20, 50]}
# Degree hyperparameters tested 
degree = {'degree': [2, 3, 4, 5, 6, 7, 8]}
# Gamma values Tested
gamma_poly = {'gamma': [0.01, 0.1, 1, 2]} # taking long to converge when i increase the gamma
gamma_rbf = {'gamma': [0.01, 0.1, 1, 5, 7, 8, 9,10]}


param_grid_rbf = C | gamma_rbf

param_grid_linear = C

param_grid_poly = degree | gamma_poly
...
def main():
    ...
    if kernel == "poly":
        results = evaluate_feature_pairs(data, features, target, kernel, output_path, output_file, save_name_mapping, param_grid = param_grid_poly, res_features=res_features_poly)
        # Display results summary
        print("Summary Evaluation Results:")
        print("polynomial degree tested: \n ", param_grid_poly["degree"])
        print("gamma hyperparameters tested: \n ", param_grid_poly["gamma"])
        print("=" * 40)
        
        for feature_x, feature_y, train_acc, test_acc, best_score, best_model, best_params in results:
            print(f"Features: {feature_x} vs {feature_y}")
            print("Best Polynomial Degree: ", best_params["degree"])
        ...
        results = evaluate_feature_pairs(data, features, target, kernel, output_path, output_file, save_name_mapping, param_grid = param_grid_rbf,  res_features=res_features_rbf)
        # Display results
        print("Summary Evaluation Results:")
        ...
        results = evaluate_feature_pairs(data, features, target, kernel, output_path, output_file, save_name_mapping, param_grid = param_grid_linear,  res_features=res_features_linear)
        # Display results
        print("Summary Evaluation Results:")
        print("C hyperparameters tested: \n ", param_grid_linear["C"])
        print("=" * 40)
